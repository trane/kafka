...
class KafkaServer(val config: KafkaConfig, time: Time = SystemTime) extends Logging {
  ...
  def startup() {
    ...
    kafkaHealthcheck = new KafkaHealthcheck(config.brokerId, config.advertisedHostName, config.advertisedPort, config.advertisedSecure, config.zkSessionTimeoutMs, zkClient)
