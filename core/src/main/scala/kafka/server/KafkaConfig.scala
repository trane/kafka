...
class KafkaConfig private (val props: VerifiableProperties) extends ZKConfig(props) {
  ...
  /* the secure flag to publish to ZooKeeper for clients to use. In IaaS environments, this may
   * need to be different from the flag to which the broker binds. If this is not set,
   * it will publish the same secure flag that the broker binds to. */
  val advertisedSecure: Boolean = props.getBoolean("advertised.secure", secure)
